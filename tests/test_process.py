...
def test_age_specific_rate_model():
    ...
    pi_usa = dismod_mr.model.covariates.predict_for(
        d, d.parameters, 'all', 'total', 'all', 'USA', 'male', 1990, 0., vars[data_type], -np.inf, np.inf)
...
def test_consistent_w_non_integral_ages():
    ...
    dm.input_data = pd.DataFrame([['m_all', 0, 100, .1]], columns=[
                                 'data_type', 'age_start', 'age_end', 'value'])
