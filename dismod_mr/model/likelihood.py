...
def beta_binom(name, pi, p, n):
    ...
    # convergence requires getting these bounds right
    p_n = mc.Uniform('p_n_%s' % name, lower=1.e4, upper=1.e9, value=1.e4)
    pi_latent = [mc.Beta('pi_latent_%s_%d' % (name, i), pi[i]*p_n, (1-pi[i])
                         * p_n, value=pi_i) for i, pi_i in enumerate(pi.value)]
