...
def age_standardize_approx(name, age_weights, mu_age, age_start, age_end, ages):
    ...
    # FIXME: Pandas bug, makes clip require __array__()
    age_start = age_start.__array__().clip(ages[0], ages[-1]) - ages[0]
...
def age_integrate_approx(name, age_weights, mu_age, age_start, age_end, ages):
    ...
    age_weights = [np.array([1.e-9+float(w_ia) for w_ia in w_i.split(';')][:-1])
