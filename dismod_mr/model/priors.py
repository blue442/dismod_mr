...
def level_constraints(name, parameters, unconstrained_mu_age, ages):
    ...
    mu_sim = similar('value_constrained_mu_age_%s' %
                     name, mu_age, unconstrained_mu_age, 0., .01, 1.e-6)
...
def covariate_level_constraints(name, model, vars, ages):
    ...
    # make sure that the range of sex covariates is included
    X_sex_min = -.5 - vars['X_shift']['x_sex']
...
def derivative_constraints(name, parameters, mu_age, ages):
    ...
    def mu_age_derivative_potential(mu_age=mu_age,
                                    increasing_a0=np.clip(
                                        parameters['increasing']['age_start']-ages[0], 0, len(ages)),
                                    increasing_a1=np.clip(
                                        parameters['increasing']['age_end']-ages[0], 0, len(ages)),
                                    decreasing_a0=np.clip(
                                        parameters['decreasing']['age_start']-ages[0], 0, len(ages)),
