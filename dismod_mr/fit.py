    ...
    if verbose:
        try:
            m.sample(m.iter, m.burn, m.thin, tune_interval=tune_interval,
                     progress_bar=True, progress_bar_fd=sys.stdout)
        except TypeError:
            m.sample(m.iter, m.burn, m.thin, tune_interval=tune_interval,
                     progress_bar=False, verbose=verbose)
    else:
...
def setup_asr_step_methods(m, vars, additional_stochs=[]):
    ...
    groups += [[g_i, g_j]
               for g_i, g_j in zip(ap_group[1:], ap_group[:-1])] + [fe_group, ap_group, fe_group+ap_group]
